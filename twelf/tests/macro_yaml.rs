...
const YAML_TEST_FILE: &str = "./tests/fixtures/test.yaml";
...
fn yaml_simple_types() {
    ...
    let prio = vec![Layer::Yaml(YAML_TEST_FILE.into()), Layer::Env(None)];
    let config = TestCfg::with_layers(&prio).unwrap();
    assert_eq!(config.test, String::from("from file"));
    assert_eq!(config.another, 5usize);

    let prio = vec![Layer::Env(None), Layer::Yaml(YAML_TEST_FILE.into())];
    let config = TestCfg::with_layers(&prio).unwrap();
    assert_eq!(config.test, String::from("from file"));
...
fn yaml_simple_with_prefix() {
    ...
    let prio = vec![
        Layer::Yaml(YAML_TEST_FILE.into()),
